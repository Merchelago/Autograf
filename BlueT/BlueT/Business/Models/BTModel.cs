...
public partial record BtModel()
{
    ...
    {
        bt = BtService;
        Search.ForEachAsync(async (_, ct) => new ValueTask(bt.Refresh(Search.Value(ct).Result, ct)));
    ...
    public IListFeed<Device> DevicesSearch => ListFeed.Async(async ct => await bt.ScanDevicesAsync(ct), bt.RefreshList)
        .Where(
                dev => 
    ...
    public IState<int> CurrentItemsDevicesSearch => State.Async(this, async ct =>
    {
        IImmutableList<Device> devices1 = await DevicesSearch.Value(ct);
        var devices = devices1.Count;
        return devices;
    }, bt.RefreshList);
